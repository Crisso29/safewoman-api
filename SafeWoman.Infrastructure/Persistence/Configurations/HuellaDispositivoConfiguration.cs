...
public class HuellaDispositivoConfiguration : IEntityTypeConfiguration<HuellaDispositivo>
{
    public void Configure(EntityTypeBuilder<HuellaDispositivo> builder)
    {
        ...
        builder.Property(h => h.FechaPrimerUso).HasColumnName("fecha_primer_uso").HasDefaultValueSql("(NOW() AT TIME ZONE 'UTC')");
        builder.Property(h => h.FechaUltimoUso).HasColumnName("fecha_ultimo_uso").HasDefaultValueSql("(NOW() AT TIME ZONE 'UTC')");
